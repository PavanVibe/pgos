...
exports.saveTenantNoteManual = exports.sendReminderManual = exports.getOverdueResidentsManual = exports.scanOverdueManual = exports.generateInvoicesManual = exports.getRoomHistory = exports.getPGComplaint = exports.getPGComplaints = exports.resolveComplaint = exports.createComplaint = exports.refundDeposit = exports.payDeposit = exports.payRent = exports.getPGRooms = exports.allocateBedController = exports.updateBedController = exports.deleteRoomController = exports.updateRoomController = exports.createRoom = exports.getOrganizationPGs = exports.updatePG = exports.createPG = void 0;
...
const updatePGSchema = zod_1.z.object({
...
const updatePG = async (req, res) => {
    try {
        ...
        const pg = await prisma_1.default.pG.update({
            where: { id: pgId },
            data: updateData
        });
...
const updateRoomController = async (req, res) => {
    try {
        ...
        const result = await prisma_1.default.$transaction(async (tx) => {
            ...
            if (isActive === false) {
                const hasActiveOccupants = room.beds.some((b) => b.tenantProfile !== null);
            ...
            if (capacity !== undefined && capacity !== room.capacity) {
                ...
                if (capacity > room.capacity) {
                    ...
                    const existingBedNumbers = new Set(currentBeds.map((b) => b.bedNumber));
                    const newBedsData = [];
                    let currentIdx = 1;
                    ...
                    const vacantBeds = currentBeds.filter((b) => b.tenantProfile === null);
                    ...
                    const bedsToRemoveIds = vacantBeds.slice(0, diff).map((b) => b.id);
                    await tx.bed.updateMany({
    ...
    try {
        ...
        if (hasHistory) {
            ...
            await prisma_1.default.$transaction([
                prisma_1.default.room.update({
                    where: { id: roomId },
                    data: { isActive: false, deletedAt: new Date() }
                }),
                prisma_1.default.bed.updateMany({
                    where: { roomId },
                    data: { isActive: false, deletedAt: new Date() }
                })
            ]);
            ...
            await prisma_1.default.$transaction([
                prisma_1.default.bed.deleteMany({
                    where: { roomId }
                }),
                prisma_1.default.room.delete({
                    where: { id: roomId }
                })
            ]);
        }
...
const updateBedController = async (req, res) => {
    try {
        ...
        const bed = await prisma_1.default.bed.findFirst({
        ...
        if (bedNumber !== undefined)
            updateData.bedNumber = bedNumber;
        if (monthlyRent !== undefined)
...
const resolveComplaint = async (req, res) => {
    try {
        ...
        const actorId = req.auth?.userId || 'system';
        const { repairCost, responsibility, assignedTenantId, billUrl, resolvedImageUrl, resolutionNotes, deductionItems, recoveryMethodInput } = req.body;
        const updatedComplaint = await ResolveComplaintWorkflow_1.ResolveComplaintWorkflow.execute(pgId, complaintId, actorId, repairCost ? parseFloat(repairCost) : undefined, responsibility, assignedTenantId, billUrl, resolvedImageUrl, resolutionNotes, deductionItems, recoveryMethodInput);
