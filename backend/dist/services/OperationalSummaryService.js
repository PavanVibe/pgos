...
exports.OperationalSummaryService = void 0;
...
class OperationalSummaryService {
     ...
     */
    static async getTasksSummary(pgId) {
        ...
        const [overdueList, unresolvedComplaints, urgentComplaints, vacantBeds, moveOutsToday, rentDueTomorrow,] = await Promise.all([
            // Fetch active unpaid rent invoices to compile operational states cleanly without duplication
            OverdueService_1.OverdueService.getOverdueResidentsList(pgId, [client_1.InvoiceStatus.PENDING, client_1.InvoiceStatus.PAST_DUE]),
        ...
        const chronicDelayTenants = Array.from(new Set(overdueList
     ...
     */
    static async getOccupancySummary(pgId) {
        const todayStart = new Date(new Date().setHours(0, 0, 0, 0));
        const todayEnd = new Date(new Date().setHours(23, 59, 59, 999));
    ...
    static async getActivityFeed(pgId, limit = 10) {
        const allLogs = await prisma_1.default.eventLog.findMany({
...
exports.OperationalSummaryService = OperationalSummaryService;
//# sourceMappingURL=OperationalSummaryService.js.map