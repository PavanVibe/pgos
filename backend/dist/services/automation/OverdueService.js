...
class OverdueService {
     ...
     */
    static async scanAndProcessOverdueInvoices(actorId = 'system') {
        console.log(`[OverdueService] Starting overdue invoice scan...`);
        const now = new Date();
        // 1. Fetch pending invoices past their due date
        const overdueInvoices = await prisma_1.default.rentInvoice.findMany({
            where: {
                status: 'PENDING',
                dueDate: {
                    lt: now
     ...
     */
    static async getOverdueResidentsList(pgId, statusFilter = [client_1.InvoiceStatus.PAST_DUE], filterType) {
        const overdueInvoices = await prisma_1.default.rentInvoice.findMany({
            where: {
                tenantProfile: { pgId },
        ...
        const lastPaidInvoices = await prisma_1.default.rentInvoice.findMany({
            where: {
        ...
        const allHistoricalInvoices = await prisma_1.default.rentInvoice.findMany({
            where: {
                pgTenantId: { in: tenantProfileIds },
        ...
        const reminderLogs = await prisma_1.default.eventLog.findMany({
            where: {
        ...
        const noteLogs = await prisma_1.default.eventLog.findMany({
            where: {
...
//# sourceMappingURL=OverdueService.js.map