...
class VacateResidentWorkflow {
    ...
    static async execute(pgId, tenantId, actorId) {
        const result = await prisma_1.default.$transaction(async (tx) => {
            ...
            const paidDepositInvoices = depositInvoices.filter(inv => inv.status === client_1.InvoiceStatus.PAID);
            const collectedDeposit = paidDepositInvoices.reduce((sum, inv) => sum + inv.amount, 0);
            const unpaidRentInvoices = profile.invoices.filter(inv => inv.type === 'RENT' && inv.status !== client_1.InvoiceStatus.PAID);
            const unpaidUtilityInvoices = profile.invoices.filter(inv => (inv.type === 'UTILITY' || inv.type === 'UTILITIES') && inv.status !== client_1.InvoiceStatus.PAID);
            ...
            for (const rentInv of unpaidRentInvoices) {
                if (remainingCollectedDeposit <= 0)
                    break;
