...
async function main() {
    ...
    const bed = await prisma_1.default.bed.findFirst({ where: { roomId: room.id } });
    if (!bed)
        throw new Error("No beds found in room.");
    ...
    const profile = await OnboardResidentWorkflow_1.OnboardResidentWorkflow.execute(pg.id, bed.id, phone, "Programmatic Razorpay Tester", "razorpay.test@pgos.com", new Date(), 10000, 15000, actorId, false, undefined, true, false, false);
    ...
    const rentLink1 = await prisma_1.default.paymentLink.findFirst({
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        const amount = 1000;
        await razorpayService_1.RazorpayService.processSuccessfulPayment(rentLink1.referenceId, txnId, amount, 'upi');
        // Verify invoice status and cumulative paid amount
        const updatedInvoice = await prisma_1.default.rentInvoice.findUnique({
            where: { id: rentInvoice.id }
        });
        if (!updatedInvoice)
            throw new Error("Invoice missing!");
        const expectedStatus = i === 10 ? client_1.InvoiceStatus.PAID : client_1.InvoiceStatus.PARTIALLY_PAID;
    ...
    // First payment
    const firstReceipt = await razorpayService_1.RazorpayService.processSuccessfulPayment(rentLink1.referenceId, duplicateTxnId, 1000, 'upi', mockWebhookEventId);
    // Duplicate process call
    const duplicateReceipt = await razorpayService_1.RazorpayService.processSuccessfulPayment(rentLink1.referenceId, duplicateTxnId, 1000, 'upi', mockWebhookEventId);
    ...
    // ----------------------------------------------------
    const depLink = await prisma_1.default.paymentLink.findFirst({
        where: { invoiceId: depositInvoice.id }
    });
    if (!depLink)
        throw new Error("Deposit invoice missing PaymentLink!");
    // Perform 5 partial payments of ₹3,000 each (representing 5 operations)
    for (let i = 1; i <= 5; i++) {
        const txnId = `pay_deposit_partial_${i}_${Date.now()}`;
        const amount = 3000;
        await razorpayService_1.RazorpayService.processSuccessfulPayment(depLink.referenceId, txnId, amount, 'upi');
        const updatedInvoice = await prisma_1.default.rentInvoice.findUnique({
            where: { id: depositInvoice.id }
        });
        if (!updatedInvoice)
            throw new Error("Deposit invoice missing!");
        const expectedStatus = i === 5 ? client_1.InvoiceStatus.PAID : client_1.InvoiceStatus.PARTIALLY_PAID;
    ...
    const recovery = await prisma_1.default.damageRecovery.create({
        data: {
            pgId: pg.id,
            tenantId: profile.id,
            roomId: room.id,
    ...
    await new Promise(resolve => setTimeout(resolve, 2000));
    const recLink = await prisma_1.default.paymentLink.findFirst({
        where: { recoveryId: recovery.id }
    });
    if (!recLink)
        throw new Error("Damage recovery did not auto-generate PaymentLink!");
    assertionCount++;
    ...
    for (let i = 1; i <= 6; i++) {
        ...
        const amount = 1000;
        await razorpayService_1.RazorpayService.processSuccessfulPayment(recLink.referenceId, txnId, amount, 'upi');
        const updatedRecovery = await prisma_1.default.damageRecovery.findUnique({
            where: { id: recovery.id }
        });
        if (!updatedRecovery)
            throw new Error("Damage recovery missing!");
    ...
    const finalProfile = await prisma_1.default.pGTenantProfile.findUnique({
        where: { id: profile.id },
    ...
    const updatedProfileAfterRefund = await prisma_1.default.pGTenantProfile.findUnique({
        where: { id: profile.id }
    });
    if (!updatedProfileAfterRefund)
        throw new Error("Profile missing after refund!");
...
//# sourceMappingURL=test-razorpay-collection.js.map