...
async function main() {
    ...
    const profile = await OnboardResidentWorkflow_1.OnboardResidentWorkflow.execute(pg.id, bedA.id, phone, "Srija Dad", "srijadad@refund.com", new Date(), 0, // rent = 0
    12000, // deposit = 12000
    actorId, false, undefined, true, // bypassEmailCheck
    false, true, // depositCollected = true
    'UPI', new Date());
    // Assert expected dues before refund
    const profileBefore = await prisma_1.default.pGTenantProfile.findUnique({
        where: { id: profile.id },
        include: {
    ...
    const profileAfter = await prisma_1.default.pGTenantProfile.findUnique({
        where: { id: profile.id },
        include: {
            invoices: { where: { isActive: true } },
            damageRecoveries: true
    ...
    if (!profileAfter)
        throw new Error("Profile not found after refund.");
    ...
    const ledgerTx = await prisma_1.default.depositLedgerTransaction.findFirst({
    ...
    if (depositRefundedAfter !== 12000)
        throw new Error("Validation Fail: Refunded amount is not ₹12,000");
    if (!ledgerTx)
        throw new Error("Validation Fail: Refund ledger transaction was not created");
    if (!isUnlocked)
        throw new Error("Validation Fail: Settlement lock is not removed after refund");
...
//# sourceMappingURL=test-refund-unlock.js.map