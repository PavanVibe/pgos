...
async function runTests() {
    ...
    const bed = await prisma_1.default.bed.findFirst({ where: { isActive: true } });
    if (!bed)
        throw new Error("No bed found.");
    // Clean up any active profile on this bed
    await prisma_1.default.pGTenantProfile.updateMany({
        where: { bedId: bed.id, status: { in: [client_1.TenantStatus.ACTIVE, client_1.TenantStatus.NOTICE, client_1.TenantStatus.INCOMPLETE] } },
    ...
    const oldTenants1 = await prisma_1.default.globalTenant.findMany({ where: { phone: phone1 } });
    for (const t of oldTenants1) {
        await prisma_1.default.pGTenantProfile.updateMany({ where: { globalTenantId: t.id }, data: { isActive: false, status: client_1.TenantStatus.PAST } });
    }
    console.log("Onboarding resident with Expected = ₹12,000, Collected = ₹12,000...");
    const profile1 = await OnboardResidentWorkflow_1.OnboardResidentWorkflow.execute(pg.id, bed.id, phone1, "Resident One (Case 1)", "res1@case1.com", new Date(), 0, // rent = 0 to isolate deposit calculations
    12000, "system_test", false, undefined, true, // bypassEmailCheck
    false, true, // depositCollected
    "UPI", new Date());
    console.log("Creating Damage Recovery of ₹20,000 for Resident One...");
    const recovery1 = await prisma_1.default.damageRecovery.create({
        data: {
            pgId: pg.id,
    ...
    const vacated1 = await VacateResidentWorkflow_1.VacateResidentWorkflow.execute(pg.id, profile1.id, "system_test");
    // Reload profile and damage recovery to assert fields
    const updatedProfile1 = await prisma_1.default.pGTenantProfile.findUnique({
        where: { id: profile1.id },
        include: { damageRecoveries: true }
    });
    const updatedRecovery1 = await prisma_1.default.damageRecovery.findUnique({
    ...
    if (updatedProfile1?.status !== client_1.TenantStatus.PAST)
        throw new Error("Case 1 Fail: Status not PAST");
    if (updatedProfile1?.depositRefundedAmount !== 0)
        throw new Error("Case 1 Fail: depositRefundedAmount is not ₹0");
    if (updatedProfile1?.depositDeductionAmount !== 12000)
        throw new Error("Case 1 Fail: depositDeductionAmount is not ₹12,000");
    if (updatedRecovery1?.status !== "PARTIALLY_RECOVERED")
        throw new Error("Case 1 Fail: Recovery status not PARTIALLY_RECOVERED");
    if (updatedRecovery1?.outstandingAmount !== 8000)
        throw new Error("Case 1 Fail: Recovery outstanding amount is not ₹8,000");
    ...
    const oldTenants2 = await prisma_1.default.globalTenant.findMany({ where: { phone: phone2 } });
    for (const t of oldTenants2) {
        await prisma_1.default.pGTenantProfile.updateMany({ where: { globalTenantId: t.id }, data: { isActive: false, status: client_1.TenantStatus.PAST } });
    }
    console.log("Onboarding resident with Expected = ₹12,000, Collected = ₹0...");
    const profile2 = await OnboardResidentWorkflow_1.OnboardResidentWorkflow.execute(pg.id, bed.id, phone2, "Resident Two (Case 2)", "res2@case2.com", new Date(), 0, // rent = 0 to isolate deposit calculations
    12000, "system_test", false, undefined, true, // bypassEmailCheck
    false, false, // depositCollected = false
    undefined, undefined);
    console.log("Executing Vacate Resident Workflow...");
    const vacated2 = await VacateResidentWorkflow_1.VacateResidentWorkflow.execute(pg.id, profile2.id, "system_test");
    const updatedProfile2 = await prisma_1.default.pGTenantProfile.findUnique({
    ...
    if (updatedProfile2?.status !== client_1.TenantStatus.PAST)
        throw new Error("Case 2 Fail: Status not PAST");
    if (updatedProfile2?.depositRefundedAmount !== 0)
        throw new Error("Case 2 Fail: depositRefundedAmount is not ₹0");
    if (updatedProfile2?.depositDeductionAmount !== 0)
        throw new Error("Case 2 Fail: depositDeductionAmount is not ₹0");
    ...
    const oldTenants3 = await prisma_1.default.globalTenant.findMany({ where: { phone: phone3 } });
    for (const t of oldTenants3) {
        await prisma_1.default.pGTenantProfile.updateMany({ where: { globalTenantId: t.id }, data: { isActive: false, status: client_1.TenantStatus.PAST } });
    }
    console.log("Onboarding resident with Expected = ₹12,000, Collected = ₹12,000...");
    const profile3 = await OnboardResidentWorkflow_1.OnboardResidentWorkflow.execute(pg.id, bed.id, phone3, "Resident Three (Case 3)", "res3@case3.com", new Date(), 0, // rent = 0 to isolate deposit calculations
    12000, "system_test", false, undefined, true, // bypassEmailCheck
    false, true, // depositCollected
    "CASH", new Date());
    console.log("Creating Damage Recovery of ₹5,000 for Resident Three...");
    const recovery3 = await prisma_1.default.damageRecovery.create({
        data: {
            pgId: pg.id,
    ...
    console.log("Executing Vacate Resident Workflow...");
    const vacated3 = await VacateResidentWorkflow_1.VacateResidentWorkflow.execute(pg.id, profile3.id, "system_test");
    const updatedProfile3 = await prisma_1.default.pGTenantProfile.findUnique({
        where: { id: profile3.id }
    });
    const updatedRecovery3 = await prisma_1.default.damageRecovery.findUnique({
    ...
    if (updatedProfile3?.status !== client_1.TenantStatus.PAST)
        throw new Error("Case 3 Fail: Status not PAST");
    if (updatedProfile3?.depositRefundedAmount !== 7000)
        throw new Error("Case 3 Fail: depositRefundedAmount is not ₹7,000");
    if (updatedProfile3?.depositDeductionAmount !== 5000)
        throw new Error("Case 3 Fail: depositDeductionAmount is not ₹5,000");
    if (updatedRecovery3?.status !== "FULLY_RECOVERED")
        throw new Error("Case 3 Fail: Recovery status not FULLY_RECOVERED");
    if (updatedRecovery3?.outstandingAmount !== 0)
        throw new Error("Case 3 Fail: Recovery outstanding amount is not ₹0");
...
//# sourceMappingURL=test-vacate-bug.js.map