...
import { refundDeposit } from './controllers/pgController';
...
async function main() {
  ...
  const initialDashboard = await getPGDashboardSummary(pg.id, pg.organizationId);
  ...
  const mockRes = {
    status: (code: number) => {
      return {
        json: (data: any) => {
          if (code >= 400) refundError = data.error;
        }
      };
    }
  } as any;

  await refundDeposit(mockReqActive, mockRes);
  if (refundError) {
  ...
  const pastProfile = await prisma.pGTenantProfile.update({
    where: { id: profile.id },
    data: { status: 'PAST', bedId: null, moveOutDate: new Date() }
  });

  // Calculate dashboard summary
  let dashboard = await getPGDashboardSummary(pg.id, pg.organizationId);
  ...
  await refundDeposit(mockReqRefundFull, mockRes);

  // Fetch updated profile
  const refundedProfile = await prisma.pGTenantProfile.findUnique({
    where: { id: profile.id }
  });
  ...
  dashboard = await getPGDashboardSummary(pg.id, pg.organizationId);
  ...
  const oldTenantsC = await prisma.globalTenant.findMany({ where: { phone: phoneC } });
  ...
  await refundDeposit(mockReqPartial, mockRes);

  const partialProfile = await prisma.pGTenantProfile.findUnique({
    where: { id: profileC.id }
  });
  ...
  await getDepositLedger({ pg: { id: pg.id } } as any, mockResLedger);

  console.log("\nLedger Verification for Sumith:");
  const sumithRow = ledgerData.find((r: any) => r.id === profileC.id);
